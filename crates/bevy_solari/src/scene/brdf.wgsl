...
#import bevy_pbr::lighting::{D_GGX, V_SmithGGXCorrelated, specular_multiscatter, F_Schlick_vec}
...
fn lobe_reflectances(F0_metal: vec3<f32>, F0_dielectric: vec3<f32>, material: ResolvedMaterial, NdotV: f32) -> LobeReflectances {
    if material.roughness <= MIRROR_ROUGHNESS_THRESHOLD {
        let F_m = F_Schlick_vec(F0_metal, 1.0, NdotV);
        let F_d = F_Schlick_vec(F0_dielectric, 1.0, NdotV);
...
fn evaluate_and_sample_brdf(
    ...
    let specular_weight = dot(rho.rho_spec, vec3(1.0 / 3.0)) / dot(rho.rho_spec + rho.rho_diff, vec3(1.0 / 3.0));
...
fn evaluate_specular_brdf(wo: vec3<f32>, wi: vec3<f32>, world_normal: vec3<f32>, material: ResolvedMaterial) -> vec3<f32> {
    ...
    if material.roughness <= MIRROR_ROUGHNESS_THRESHOLD {
        if abs(NdotH - 1.0) < 0.0001 {
            let F_m = F_Schlick_vec(F0_metal, 1.0, HdotV);
            let F_d = F_Schlick_vec(F0_dielectric, 1.0, HdotV);
